...
engine and the static UI (worlds are generated, `stock/sim/worldgen`); `stock/api/
...
scratch goes to `.pyinstaller/` (never `build/`, which holds the build docs).
"""
...
def main() -> None:
    ...
    PyInstaller.__main__.run(
        [
            ...
            "--workpath", str(SCRATCH / "work"),
            "--specpath", str(SCRATCH),
            "--add-data", f"{ROOT / 'stock' / 'ui' / 'static'}{sep}stock/ui/static",
